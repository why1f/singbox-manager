...
### Added
- **TUI 顶部 Tab 支持鼠标精确点击切页**：点击命中改为按真实标签宽度判断，不再按整行等分，窗口化和全屏下都更准确。
- **仪表盘摘要区支持鼠标滚轮滚动**：可分别在“用户摘要”和“节点摘要”区域内滚动查看内容。

### Changed
- **仪表盘摘要布局按内容自适应**：用户摘要不再在少量用户时占用过多空白，节点摘要会获得更多可视空间；内容超出时可直接滚动查看。
