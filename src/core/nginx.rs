...
fn systemctl(action: &str) -> Result<()> {
    ...
    if status.success() {
        ...
        Err(anyhow!("systemctl {} nginx 失败", action))
    }
}
...
pub fn generate_conf(conf_path: &str, public_base: &str, upstream_listen: &str) -> Result<()> {
    ...
    if host.is_empty() {
        return Err(anyhow!(
            "public_base 为空，先在 config.toml 填上 [subscription].public_base"
        ));
    }
...
"#,
        host = host,
        upstream = upstream_listen,
    );

    if let Some(p) = Path::new(conf_path).parent() {
        std::fs::create_dir_all(p)
            .with_context(|| format!("创建 nginx 配置目录 {} 失败", p.display()))?;
    }
    std::fs::write(conf_path, template).with_context(|| format!("写入 {} 失败", conf_path))?;
    Ok(())
}
