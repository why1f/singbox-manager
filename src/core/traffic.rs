use crate::model::{traffic::{LiveTrafficSnapshot, TrafficDelta}, user::User};
...
pub fn calc_deltas(snaps: &[LiveTrafficSnapshot], users: &[User]) -> Vec<TrafficDelta> {
    snaps.iter().filter_map(|s| {
        ...
        if du == 0 && dd == 0 { return None; }
        Some(TrafficDelta {
            username: s.username.clone(), delta_up: du as i64, delta_down: dd as i64,
            new_live_up: s.up_bytes as i64, new_live_down: s.down_bytes as i64,
        })
    }).collect()
}
...
mod tests {
    use super::*;
    #[test] fn normal()   { assert_eq!(calc_delta(1000, 600),  400); }
    #[test] fn no_change(){ assert_eq!(calc_delta(600,  600),  0);   }
    #[test] fn restart()  { assert_eq!(calc_delta(200,  5000), 200); }
}
