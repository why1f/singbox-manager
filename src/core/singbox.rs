...
impl SingboxProcess {
    ...
    pub fn start(&self) -> Result<()> { systemctl("start") }
    pub fn stop(&self)  -> Result<()> { systemctl("stop") }
...
fn is_enabled() -> bool {
    Command::new("systemctl").args(["is-enabled", "sing-box"])
...
pub async fn install_v2rayapi(repo: &str) -> Result<()> {
    ...
    let tag = releases.as_array()
        .and_then(|arr| arr.iter().find_map(|r| {
            let t = r["tag_name"].as_str()?;
            if t.starts_with("singbox-") && !r["prerelease"].as_bool().unwrap_or(false) {
                Some(t.to_string())
            } else { None }
        }))
        .ok_or_else(|| anyhow!("仓库 {} 里找不到 singbox-* release，请先跑一次 build-singbox workflow", repo))?;

    let version = tag.trim_start_matches("singbox-");
    let asset = format!("sing-box-{}-{}-v2rayapi.tar.gz", version, arch);
    let download_url = format!("https://github.com/{}/releases/download/{}/{}", repo, tag, asset);
    ...
    let status = Command::new("tar")
        ...
        .arg("-C").arg(&tmp_dir)
        .status()?;
    if !status.success() { return Err(anyhow!("解压 tarball 失败")); }

    let inner = tmp_dir.join(format!("sing-box-{}-{}-v2rayapi", version, arch));
    let src_bin = inner.join("sing-box");
    if !src_bin.exists() { return Err(anyhow!("tarball 内未找到 sing-box 二进制")); }
    ...
    let cfg_dir = std::path::Path::new("/etc/sing-box");
    if !cfg_dir.exists() { std::fs::create_dir_all(cfg_dir)?; }
    let cfg_file = cfg_dir.join("config.json");
    if !cfg_file.exists() {
...
fn verify_sha256(file: &std::path::Path, sha_line: &str) -> Result<()> {
    let expected = sha_line.split_whitespace().next().unwrap_or("").to_ascii_lowercase();
    if expected.len() != 64 { return Ok(()); } // 非法 sha 文件，跳过校验
    let out = Command::new("sha256sum").arg(file).output()?;
    if !out.status.success() { return Ok(()); }
    let got = String::from_utf8_lossy(&out.stdout);
    let got = got.split_whitespace().next().unwrap_or("").to_ascii_lowercase();
    if got != expected {
        return Err(anyhow!("sha256 校验失败: expected {} got {}", expected, got));
    }
    Ok(())
}
...
#[cfg(not(unix))]
fn set_executable(_: &std::path::Path) -> Result<()> { Ok(()) }
