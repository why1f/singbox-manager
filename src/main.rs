...
async fn main() -> Result<()> {
    ...
    let cfg = load_or_init_config(
        &cfg_path,
        matches!(&command, Commands::Daemon | Commands::Tui),
    )?;

    match command {
        Commands::Users => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::List, &pool, &cfg).await
        }
        Commands::Add(a) => {
            let pool = open_pool(&cfg).await?;
            run_user(
                cli::user::UserCommands::Add {
                    name: a.name,
                    quota: a.quota,
                    reset_day: a.reset_day,
                    expire: a.expire,
        ...
        Commands::Del { name } => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::Del { name }, &pool, &cfg).await
        }
        Commands::On { name } => {
            let pool = open_pool(&cfg).await?;
            set_user_enabled(&pool, &name, true, &cfg).await
        }
        Commands::Off { name } => {
            let pool = open_pool(&cfg).await?;
            set_user_enabled(&pool, &name, false, &cfg).await
        }
        Commands::Reset { name } => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::Reset { name }, &pool, &cfg).await
        }
        Commands::Info { name } => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::Info { name }, &pool, &cfg).await
        }
        Commands::Sub { name } => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::Sub { name }, &pool, &cfg).await
        }
        Commands::Pkg(a) => {
            let pool = open_pool(&cfg).await?;
            run_user(
                cli::user::UserCommands::Package {
                    name: a.name,
                    quota: a.quota,
                    reset_day: a.reset_day,
                    expire: a.expire,
        ...
        Commands::Grant { name, tag } => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::Grant { name, tag }, &pool, &cfg).await
        }
        Commands::Revoke { name, tag } => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::Revoke { name, tag }, &pool, &cfg).await
        }
        Commands::GrantAll { name } => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::GrantAll { name }, &pool, &cfg).await
        }
        Commands::Allowed { name } => {
            let pool = open_pool(&cfg).await?;
            run_user(cli::user::UserCommands::Allowed { name }, &pool, &cfg).await
        }
        ...
        Commands::User(a) => {
            let pool = open_pool(&cfg).await?;
            run_user(a.command, &pool, &cfg).await
        }
...
fn resolve_config_path(arg: Option<&str>) -> PathBuf {
    ...
    if let Ok(p) = std::env::var("SB_MANAGER_CONFIG") {
        return PathBuf::from(p);
    }
...
fn load_or_init_config(path: &Path, allow_create: bool) -> Result<AppConfig> {
    if path.exists() {
        ...
        anyhow::bail!(
            "配置文件不存在: {} （使用 --config 指定或运行 tui/daemon 自动生成）",
            path.display()
        )
    }
}

async fn open_pool(cfg: &AppConfig) -> Result<sqlx::SqlitePool> {
    if let Some(parent) = Path::new(&cfg.db.path).parent() {
        std::fs::create_dir_all(parent)
            .with_context(|| format!("创建数据目录 {} 失败", parent.display()))?;
    }
    db::init_pool(&cfg.db.path).await
}
...
async fn run_tui(pool: sqlx::SqlitePool, cfg: AppConfig) -> Result<()> {
    ...
    // UI Tick 节拍（1s，总是有）；不依赖 gRPC
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                iv.tick().await;
                if tx_tick.send(TrafficEvent::Tick).await.is_err() {
                    break;
                }
            }
        });
...
async fn run_user(
    ...
    match cmd {
            ...
            for u in &us {
                println!(
                    ...
                    if u.quota_gb <= 0.0 {
                        ...
                        format!("{}GB", u.quota_gb)
                    },
                    if u.expire_at.is_empty() {
                        "永久".into()
                    } else {
                        u.expire_at.clone()
                    }
        ...
        UserCommands::Add {
            ...
            let u =
                user_service::add_user(pool, &name, quota, reset_day, &expire, multiplier).await?;
...
async fn set_user_enabled(
    ...
    let user = db::user_repo::get(pool, name)
        .await?
        .ok_or_else(|| anyhow::anyhow!("用户不存在: {}", name))?;
...
async fn run_kernel(cmd: cli::kernel::KernelCommands, cfg: &AppConfig) -> Result<()> {
    ...
    match cmd {
        ...
        K::Enable => {
            core::singbox::enable()?;
            println!("✓ sing-box 已设为开机自启");
        }
        K::Disable => {
            core::singbox::disable()?;
            println!("✓ sing-box 已关闭开机自启");
        }
    }
    Ok(())
}
...
async fn run_status(cfg: &AppConfig) -> Result<()> {
    ...
    let grpc = if core::grpc::connect(&cfg.singbox.grpc_addr).await.is_ok() {
        "已连接"
    } else {
        "未连接"
    };
...
async fn run_doctor(cfg_path: &Path, cfg: &AppConfig) -> Result<()> {
    let report = core::doctor::run(cfg_path, cfg).await;
    for item in &report.items {
        let level = match item.level {
            core::doctor::CheckLevel::Ok => "[ OK ]",
            core::doctor::CheckLevel::Warn => "[WARN]",
            core::doctor::CheckLevel::Error => "[ERR ]",
        };
        println!("{:<6} {:<14} {}", level, item.label, item.detail);
    }
    println!(
        "\n汇总: {} OK / {} WARN / {} ERR",
        report.ok_count(),
    ...
    if report.has_errors() {
        anyhow::bail!("doctor 检查发现错误项");
    }
    Ok(())
}

async fn run_token(
    cmd: cli::token::TokenCommands,
    pool: &sqlx::SqlitePool,
    cfg: &AppConfig,
) -> Result<()> {
...
fn run_nginx(cmd: cli::nginx::NginxCommands, cfg: &AppConfig) -> Result<()> {
    ...
    match cmd {
        ...
        N::Test => {
            let out = core::nginx::test_config()?;
            println!("{}", out);
        }
        N::GenConf => {
