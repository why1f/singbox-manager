use ratatui::{Frame,layout::Rect,style::{Color,Style},text::{Line,Span},
...
pub fn render(f: &mut Frame, area: Rect, s: &AppState) {
    ...
    lines.extend(s.log_lines.iter().skip(skip).map(|l| {
        let c = if l.contains("ERROR")||l.contains("错误"){Color::Red}
                else if l.contains("WARN")||l.contains("警告"){Color::Yellow} else{Color::White};
        Line::from(Span::styled(l.clone(),Style::default().fg(c)))
    }));
    f.render_widget(Paragraph::new(lines)
        .block(Block::default().borders(Borders::ALL).title(" 日志 / 输出 ")).wrap(Wrap{trim:false}), area);
}
