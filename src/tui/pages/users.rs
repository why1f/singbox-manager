...
pub fn render(f: &mut Frame, area: Rect, s: &AppState) {
    ...
    let rows: Vec<Row> = s
        ...
        .map(|(i, u)| {
            ...
            let exp = if u.expire_at.is_empty() {
                "永久".into()
            } else {
                u.expire_at.clone()
            };
            ...
            let billing = if (u.traffic_multiplier - 2.0).abs() < 0.01 {
                "双向".to_string()
            } else if (u.traffic_multiplier - 1.0).abs() < 0.01 {
                "单向".to_string()
            } else {
                format!("{:.1}x", u.traffic_multiplier)
            };
            Row::new(vec![
                Cell::from(u.name.clone()),
                Cell::from(if u.enabled {
                    "● 启用"
                } else {
                    "○ 禁用"
                })
    ...
    f.render_widget(
        ...
        .header(hdr)
        .block(Block::default().borders(Borders::ALL).title(" 用户列表 ")),
        c[0],
    );

    let sel_text = s
        .users
        .get(s.user_table.selected)
        .map(|u| {
            let tags = if u.allow_all_nodes {
                "全部".to_string()
            } else {
