...
async fn event_loop(
    ...
    s.nginx_public_base = if cfg.subscription.public_base.is_empty() {
        ...
        Some(cfg.subscription.public_base.clone())
    };
    ...
    match proc.check_config() {
        Ok(()) => {
            if matches!(proc.is_running(), Some(true)) {
                ...
                Some((
                    "config.json 校验通过（sing-box 未运行，跳过 reload）".into(),
                    StatusLevel::Warn,
                ))
            }
        }
        Err(e) => Some((
            format!("config.json 校验失败: {}（未 reload，请修复）", e),
            StatusLevel::Error,
        )),
    }
}
...
fn apply_ui_event(s: &mut AppState, ev: UiEvent) {
    match ev {
        UiEvent::UsersRefreshed(u) => {
            s.users = u;
            s.user_table.clamp(s.users.len());
        }
        UiEvent::NodesRefreshed(n) => {
            s.nodes = n;
        ...
        UiEvent::SysMetrics { cpu, rx, tx } => {
            ...
            if s.net_rx_history.len() > 60 {
                s.net_rx_history.remove(0);
            }
            s.net_tx_history.push(tx);
            if s.net_tx_history.len() > 60 {
                s.net_tx_history.remove(0);
            }
        }
    }
}
...
fn apply_traffic_event(s: &mut AppState, ev: TrafficEvent) {
    match ev {
        ...
        TrafficEvent::Synced(deltas) => {
            ...
            if s.traffic_history.len() > 60 {
                s.traffic_history.remove(0);
            }
            s.last_sync_time = Some(chrono::Local::now());
            s.grpc_connected = true;
        }
        TrafficEvent::GrpcConnected => {
            s.grpc_connected = true;
        }
...
fn handle_page_key(
    ...
    match k.code {
        ...
        KeyCode::Char('a') => match s.page {
            ...
            Page::Nodes => {
                s.modal = Some(Modal::AddNode(NodeForm {
                    port: "443".into(),
                    ..NodeForm::default()
                }))
            }
            ...
            Page::Users => {
                if let Some(u) = s.selected_user() {
                    s.modal = Some(Modal::EditUser(crate::tui::forms::UserEditForm {
                        name: u.name.clone(),
                        quota: if u.quota_gb <= 0.0 {
                            String::new()
                        } else {
                            format!("{}", u.quota_gb)
                        },
                        reset_day: if u.reset_day == 0 {
                            String::new()
                        } else {
                            u.reset_day.to_string()
                        },
        ...
        KeyCode::Char('d') => match s.page {
            ...
            Page::Nodes => {
                if let Some(tag) = s.selected_node().map(|n| n.tag.clone()) {
        ...
        KeyCode::Char('r') if s.page == Page::Users => {
            if let Some(name) = s.selected_user().map(|u| u.name.clone()) {
                s.modal = Some(Modal::ConfirmResetUser(name));
            }
        }
        KeyCode::Char('s') if s.page == Page::Users => {
            if let Some(name) = s.selected_user().map(|u| u.name.clone()) {
                spawn_export(cfg, ui_tx, name);
            }
        }
        KeyCode::Char('u') if s.page == Page::Users => {
            if let Some(u) = s.selected_user() {
                if u.sub_token.is_empty() {
                    ...
                    s.set_status(
                        format!(
                            "需在 config.toml 里填 [subscription].public_base；当前 token: {}",
                            u.sub_token
                        ),
                        StatusLevel::Warn,
                    );
...
fn handle_nginx_key(
    ...
    match k.code {
        ...
        KeyCode::Char('i') => spawn_nginx_op(
            ui_tx,
            cfg,
            "安装 nginx",
            crate::core::nginx::install_via_pkg,
        ),
        ...
        KeyCode::Char('l') => {
            spawn_nginx_op(ui_tx, cfg, "reload nginx", crate::core::nginx::reload)
        }
        KeyCode::Char('e') => {
            spawn_nginx_op(ui_tx, cfg, "nginx 开机自启", crate::core::nginx::enable)
        }
        KeyCode::Char('d') => {
            spawn_nginx_op(ui_tx, cfg, "nginx 取消自启", crate::core::nginx::disable)
        }
...
fn spawn_nginx_refresh(tx: mpsc::Sender<UiEvent>, cfg: Arc<AppConfig>) {
    tokio::spawn(async move {
        ...
        let status = tokio::task::spawn_blocking(move || crate::core::nginx::status(&conf_path))
            .await
...
fn spawn_nginx_genconf(tx: mpsc::Sender<UiEvent>, cfg: Arc<AppConfig>) {
    tokio::spawn(async move {
        ...
        let (msg, level) = match res {
            Ok(Ok(())) => (
                "✓ 反代配置已生成；编辑证书路径后 [t] 检查 + [l] reload".into(),
                StatusLevel::Warn,
            ),
...
fn spawn_save_nodes(
    ...
    tokio::spawn(async move {
        ...
        let msg = if all {
            format!("{} 已恢复全部节点可用", user)
        } else {
            format!("{} 已分配 {} 个节点", user, tags.len())
        };
        let _ = tx
            .send(UiEvent::Status {
                msg,
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            ...
            Err(e) => {
                let _ = tx
                    .send(UiEvent::Status {
                        msg: format!("节点已删除，但清理用户节点引用失败: {}", e),
                        level: StatusLevel::Error,
                    })
                    .await;
                return;
        ...
        let (msg, level) = match check_msg {
            Some(w) => (w, StatusLevel::Error),
            None if cleaned > 0 => (
                format!("已删除节点 {}，并清理 {} 个用户的分配引用", tag, cleaned),
                StatusLevel::Warn,
            ),
    ...
    tokio::spawn(async move {
        match crate::core::backup::restore_backup(&file) {
            Ok(()) => {
                let _ = tx
                    .send(UiEvent::Status {
                        msg: format!("已成功恢复备份: {}，若有路径更改建议重启服务", file),
                        level: StatusLevel::Warn,
                    })
                    .await;
            }
