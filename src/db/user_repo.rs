...
pub async fn insert(pool: &SqlitePool, u: &User) -> Result<()> {
    ...
    sqlx::query(r#"INSERT INTO users(name,uuid,password,enabled,quota_gb,used_up_bytes,used_down_bytes,
        ...
        .bind(&u.allowed_nodes).bind(&u.sub_token).bind(u.traffic_multiplier)
        .execute(pool).await?;
    Ok(())
}
...
pub async fn update_package(
    ...
    quota_gb: Option<f64>, reset_day: Option<i64>, expire_at: Option<&str>, traffic_multiplier: Option<f64>,
) -> Result<()> {
    sqlx::query(r#"UPDATE users SET
        ...
        WHERE name = ?"#)
        .bind(quota_gb).bind(reset_day).bind(expire_at).bind(traffic_multiplier).bind(name)
        .execute(pool).await?;
    Ok(())
}
